        ...
        if (!rst_n || !capture_enable_cam) begin
            ...
            if (frame_start) begin
                if (frame_seen && !drop_frame) begin
                    ...
                    // FIFO head and is consumed by an idle write arbiter,
    ...
    always @(posedge ui_clk) begin
        if (ui_rst) begin
            ...
            if (fifo_rd_en && fifo_is_marker) begin
                valid_bank_ui <= fifo_marker_bank;
                bank_valid_ui <= 1'b1;
            end
        end
    end
...
module EoV19DdrReplay #(
    ...
    input  wire        bank_valid0,
    input  wire        bank_valid1,
    input  wire        bank_valid2,
    input  wire        bank_valid3,
    input  wire        bank_valid4,
    input  wire        bank_valid5,
    input  wire        bank0,
    input  wire        bank1,
    input  wire        bank2,
    input  wire        bank3,
    ...
    assign banks_ready = bank_valid0 & bank_valid1 & bank_valid2 &
                         bank_valid3 & bank_valid4 & bank_valid5;
    ...
    function bank_for_cam;
        input [2:0] cam;
        begin
            case (cam)
                3'd0: bank_for_cam = latched_bank[0];
                3'd1: bank_for_cam = latched_bank[1];
                3'd2: bank_for_cam = latched_bank[2];
                3'd3: bank_for_cam = latched_bank[3];
                3'd4: bank_for_cam = latched_bank[4];
                default: bank_for_cam = latched_bank[5];
            endcase
        end
    endfunction
    ...
    function [28:0] cam_addr;
        ...
        begin
            cam_addr = cam_base(cam) +
                       (bank_for_cam(cam) ? FRAME_STRIDE_ADDR : 29'd0) +
                       row_base_addr +
