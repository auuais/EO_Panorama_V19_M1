...
set ltxfile [file join $root EO_Panorama_V19_M1.runs impl_1 debug_nets_repaired.ltx]
set stamp [clock format [clock seconds] -format "%Y%m%d_%H%M%S"]
set outcsv [file join $root captures usb0_v19 "ila_content_row51_replay_hsync_repaired_$stamp.csv"]
file mkdir [file dirname $outcsv]

if {![file exists $ltxfile]} { error "Repaired LTX not found: $ltxfile" }
...
foreach cand [get_hw_ilas -of_objects $dev] {
    if {[llength [get_hw_probes -of_objects $cand -filter {NAME =~ "*copy_px_data*"} -quiet]] > 0} {
        set ila $cand
        break
    }
}
if {$ila eq ""} { error "ILA with copy_px_data probe not found" }
set trig [get_hw_probes -of_objects $ila -filter {NAME =~ "*v19_content_row51"} -quiet]
if {$trig eq ""} { error "v19_content_row51 trigger probe not found" }
