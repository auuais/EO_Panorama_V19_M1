...
set outcsv [file join $root captures usb0_v19 "ila_v19_src_rd_valid_linecache_widthretire_$stamp.csv"]
...
foreach cand [get_hw_ilas -of_objects $dev] {
    if {[llength [get_hw_probes -of_objects $cand -filter {NAME =~ "*v19_src_rd_valid"} -quiet]] > 0} {
        set ila $cand
        break
    }
}
if {$ila eq ""} {
    puts "ERROR: ILA with v19_src_rd_valid probe not found"
    close_hw_target
    close_hw_manager
    exit 1
}
set trig [get_hw_probes -of_objects $ila -filter {NAME =~ "*v19_src_rd_valid"} -quiet]
